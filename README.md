...
> - **带默认值的 读** ：
> int num = xini_file_t["分节"]["键"] (1000);
> // 若键值为 空 时，取默认值 1000
> - **带默认值的 读**，**并同步写入该默认值** ：
> int num = xini_file_t["分节"]["键"].try_value(1000);
> // 若键值为 空 时，返回默认值 1000，并设置该键值为 1000
> // 这种方式，在外部未配置 INI 文件时，程序退出后，
> // 用户通过再次查看生成的 INI 文件，就可以同步了解到代码中 该键值的默认值
